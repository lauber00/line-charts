...
hoverOneHtml series system toX toY properties hovered =
    let
        x =
            Maybe.withDefault (middle .x system) (toX hovered)

        y =
            Maybe.withDefault (middle .y system) (toY hovered)

        viewHeaderOne =
            Utils.viewMaybe (findSeries hovered series) <|
                \( color, label, _ ) ->
                    viewHeader
        ...
        first :: rest ->
            Config <|
                \series toX toY system ->
                    let
                        xValue =
                            Maybe.withDefault 0 (toX first)
                    in
                    -- TODO Maybe should happen - make it not.
...
hoverManyHtml system toX toY formatX formatY first hovered series =
    let
        x =
            Maybe.withDefault (middle .x system) (toX first)

        viewValue ( color, label, data ) =
            Utils.viewMaybe (find hovered data) <|
                \hovered_ ->
                    viewTableRow (Color.toCssString color) label (formatY hovered_)
    in
    hover system x [] <|
        viewHeader [ Html.text (formatX first) ]
            :: [ Html.table [] (List.map viewValue series) ]
...
viewTableRow : String -> String -> String -> Html.Html msg
viewTableRow color label value =
    Html.tr
        []
        [ Html.td
            []
...
hoverAt system x y styles view =
    let
        ...
        xPercentage =
            (Coordinate.toSvgX system x + space) * 100 / system.frame.size.width

        yPercentage =
            Coordinate.toSvgY system y * 100 / system.frame.size.height
